...
## doFit_cutoff
```{r}
doFit_cutoff <- function(v, folds, data){ 
  
  cutoff_df = data[-folds[[v]],]
  
  pred = prediction(as.numeric(cutoff_df$MFI), as.factor(cutoff_df$outcome))
...
xval_cutoff_fxn = function(fdf, V=5, out = 180){
  ...
  predictions <- unlist(sapply(seq(V), doFit_cutoff, folds = folds, data = rdf)) #CV train/predict
