...
combo_fxn = function(fdf, nag){
  ...
  ab_combo <- as_tibble(
    ...
    stringsAsFactors = F) %>% 
    rename_with(~ paste0("ag", substr(., 2,2))) %>% 
    mutate(nag = nag,
           comboid = row_number()) %>% 
  ...
  #for (i in 1:10){ #1:nrow(ab_combo_all)
    
  for (i in 1:nrow(ab_combo)){ #1:nrow(ab_combo_all)
...
ag_name_vect = levels(factor(df$ag)) 

ab_combo_n1 <- as_tibble(
  t(combn(ag_name_vect, 1)), 
  stringsAsFactors = F, 
  .name_repair = "unique") %>% 
  mutate(ag1 = `...1`,
         ag2 = 'blank',
         ...
         nag = 1,
         comboid = row_number()) %>% 
  select(nag, comboid, ag1, ag2, ag3, ag4, ag5)

ab_combo_n2 <- as_tibble(
  t(combn(ag_name_vect, 2)), 
  stringsAsFactors = F, 
  .name_repair = "unique") %>% 
  mutate(ag1 = `...1`,
         ag2 = `...2`,
...
ab_combo_all = rbind(ab_combo_n1,
                     ab_combo_n2,
                     ab_combo_n3,
                     ab_combo_n4,
                     ab_combo_n5)

# run rf on each combination
combo_rf_list = list()

for (i in 1:10){ #1:nrow(ab_combo_all)

#for (i in 1:nrow(ab_combo_all)){ #1:nrow(ab_combo_all)
  
  cdf = df %>% filter(ag %in% c(ab_combo_all$ag1[i],
                                   ab_combo_all$ag2[i],
                                   ab_combo_all$ag3[i],
                                   ab_combo_all$ag4[i],
                                   ab_combo_all$ag5[i]))
  ...
  c_rf = xval_rf_fxn(cdf) %>% 
    mutate(nag = ab_combo_all$nag[i],
           comboid = ab_combo_all$comboid[i],
           ag1 = ab_combo_all$ag1[i],
           ag2 = ab_combo_all$ag2[i],
           ag3 = ab_combo_all$ag3[i],
           ag4 = ab_combo_all$ag4[i],
           ag5 = ab_combo_all$ag5[i])
  
  combo_rf_list[[i]] = c_rf
}

all_combo_results = bind_rows(combo_rf_list) 

all_combo_results_neat = all_combo_results %>% 
  select(nag, comboid, auc,sens95) %>% distinct() %>% 
  left_join(ab_combo_all)



write_rds(all_combo_results, 
          paste0("~/Library/CloudStorage/Box-Box/SerologyR01/Jiangsu/Jo paper/combos/combo_dfs/allcombos_", 
                 substr(gsub("\\D", "", Sys.time()), 1,14), ".rds"))
```
